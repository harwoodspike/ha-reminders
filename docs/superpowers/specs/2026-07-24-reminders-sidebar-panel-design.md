...
frontend.async_register_built_in_panel(
    ...
    config={"_panel_custom": {
        ...
        "module_url": "/ha_reminders/ha-reminders-panel.js",
        "embed_iframe": False,
...
+ static path). No version bump required.

## Data Flow

1. Integration loads → `async_setup` registers the static JS path and the custom
   panel → "Reminders" appears in the sidebar.
...
- `tests/components/ha_reminders/test_init.py`: update/replace the tests that
  ...
  `reminders`) with the expected title/icon and `_panel_custom` module URL.
